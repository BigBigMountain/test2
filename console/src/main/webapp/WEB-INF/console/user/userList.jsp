		...
		<c:forEach items="${userList}" var="user" >
			<form action="/user/saveOrUpdateUser.do" method="post">
				<tr bgcolor="#ffffff" onmouseover="this.bgColor='#C1F2A0'" onmouseout="this.bgColor='#ffffff'">
					...
					<td align="center">
						<select name="zoneId" style="height:30px;font-size:18px;border: 0px;background: none; " onmouseover="inputOver(this)" onmouseout="inputOut(this)">
							<!-- <option value="">--请选择区域--</option> -->
							<c:forEach items="${zoneList }" var="zone">
								<option value="${zone.zoneId }" <c:if test="${user.zoneId == zone.zoneId }">selected="selected"</c:if>>${zone.zoneName }</option>
							</c:forEach>
						</select>	
					</td>
					<td align="center">
	...
	<form action="/user/saveOrUpdateUser.do" method="post">
		<table style="width: 100%; height:250px;font-size:18px">
			...
			<tr>
				...
				<td>
					<select name="zoneId" style="height:30px;font-size:18px;border: 0px;background-color: #FEFFD0; " onmouseover="inputOver(this)" onmouseout="inputOut(this)">
						<option value="">--请选择区域--</option>
						<c:forEach items="${zoneList }" var="zone">
