...
//	}
	...
	public String saveOrUpdate(Model model,HttpSession session,LedProgram ledProgram,LedTable ledTable,LedHouse ledHouse,LedClimatic ledClimatic){
		
		User user = (User) session.getAttribute("loginUser");
		String msg;
		if(null == ledProgram.getP_id() || ledProgram.getP_id()==0){
			ledProgram.setZ_id(user.getZoneId());
			msg = ledService.add(ledProgram, ledTable, ledHouse, ledClimatic);
		}else{
			msg = ledService.update(ledProgram, ledTable, ledHouse, ledClimatic);
		}
