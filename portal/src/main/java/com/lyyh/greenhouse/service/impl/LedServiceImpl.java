					...
					for (int i = 0; i < houseDatas.size();) {
						...
						for (int j = 0; j < n; j++, i++) {
							...
							if (ledHouse.getH_t1() == 1) {
								...
								drawString(graphics,houseData.getTemperature(), vx, yy);
							...
							if (ledHouse.getH_t2() == 1) {
								...
								// 数据
								vx = x + 3 * cw / 4 + tos / 2 - str.length() * ts / 4;// 数据value光标
								drawString(graphics,houseData.getTemperature2(), vx, yy);
							...
							if (ledHouse.getH_h1() == 1) {
								...
								// 数据
								vx = x + 3 * cw / 4 + tos / 2 - str.length() * ts / 4;// 数据value光标
								drawString(graphics,houseData.getHumidity(), vx, yy);
							...
							if (ledHouse.getH_h2() == 1) {
								...
								// 数据
								vx = x + 3 * cw / 4 + tos / 2 - str.length() * ts / 4;// 数据value光标
								drawString(graphics,houseData.getHumidity2(), vx, yy);
							...
							if (ledHouse.getH_l() == 1) {
								...
								// 数据
								vx = x + 3 * cw / 4 + tos / 2 - str.length() * ts / 4;// 数据value光标
								drawString(graphics,houseData.getLighting(), vx, yy);
							...
							if (ledHouse.getH_c() == 1) {
								...
								// 数据
								vx = x + 3 * cw / 4 + tos / 2 - str.length() * ts / 4;// 数据value光标
								drawString(graphics,houseData.getCo2(), vx, yy);
							...
							if (ledHouse.getH_st() == 1) {
								...
								// 数据
								vx = x + 3 * cw / 4 + tos / 2 - str.length() * ts / 4;// 数据value光标
								drawString(graphics,houseData.getSoilTemperature(), vx, yy);
							...
							if (ledHouse.getH_sh() == 1) {
								...
								// 数据
								vx = x + 3 * cw / 4 + tos / 2 - str.length() * ts / 4;// 数据value光标
								drawString(graphics,houseData.getSoilHumidity(), vx, yy);
...
//				System.out.println(tname);
	...
	private void drawClimatic(Climatic newestClimatic, LedClimatic ledClimatic, Graphics graphics) {
		...
		if (ledClimatic.getC_rs() == 1) {
			drawString(graphics,newestClimatic.getRainAndSnow(),"雨雪：", ledClimatic.getC_rsx(), ledClimatic.getC_rsy());
		}
		...
		if (ledClimatic.getC_wd() == 1) {
			drawString(graphics,newestClimatic.getWindDirection(),"风向：", ledClimatic.getC_wdx(), ledClimatic.getC_wdy());
		}
